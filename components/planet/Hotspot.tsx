...
export function Hotspot({
    ...
    useFrame((state) => {
        ...
        if (outerSphereRef.current) {
            const opacity = (Math.sin(t * 2) + 1) / 2 * 0.5; // 0 to 0.5
            const mat = outerSphereRef.current.material as THREE.MeshBasicMaterial;
            mat.opacity = opacity;
        }

        if (groupRef.current) {
            // Look away from center so it's flush to surface
    ...
    return (
        <group ref={groupRef} position={position} onPointerDown={handlePointerDown} className="cursor-pointer">
            ...
            <mesh ref={outerSphereRef}>
                ...
                <meshBasicMaterial color="#fbbf24" transparent={true} opacity={0.5} blending={THREE.AdditiveBlending} depthTest={false} />
            </mesh>
            ...
            <Html distanceFactor={10} occlude>
                <div className="pointer-events-none transform -translate-x-1/2 -translate-y-[200%] transition-opacity duration-300 opacity-0 group-hover:opacity-100 peer-hover:opacity-100">
                    <div className="bg-void/80 backdrop-blur border border-accent-star/50 text-accent-star font-mono text-[8px] tracking-widest px-2 py-1 rounded whitespace-nowrap shadow-[0_0_10px_rgba(251,191,36,0.3)]">
                        MENSAJE #{message.id}
                    </div>
                </div>
            </Html>
